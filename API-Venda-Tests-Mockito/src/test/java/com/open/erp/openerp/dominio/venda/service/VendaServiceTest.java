...
public class VendaServiceTest extends AplicationConfigTest {
    ...
    public void deveRemoverVenda() {
        ...
        when(venda.getValorRecebido()).thenReturn(BigDecimal.TEN);
        when(venda.getItens()).thenReturn(Collections.emptyList());
        when(venda.getValorTotal()).thenReturn(BigDecimal.TEN);
        Optional<Venda> vendaOptoOptional = Optional.of(venda);

        when(repository.findById(ArgumentMatchers.eq(vendaId))).thenReturn(vendaOptoOptional);

        vendaService.removerVenda(vendaId);
        verify(repository, times(1)).delete(ArgumentMatchers.any(Venda.class));
        verify(estoqueService, never()).adicionarNoEstoque(ArgumentMatchers.any(), ArgumentMatchers.any());
        verify(titulosReceberService, never()).removerTitulo(ArgumentMatchers.any());
    }
